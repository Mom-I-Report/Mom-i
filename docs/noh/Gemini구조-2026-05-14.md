...
`gemini_client.py`는 모듈 로드 시 4개 프롬프트 파일을 `---` 구분자로 이어붙여 단일 `system_instruction`으로 조립한다.
...
> `age_policy.md`는 삭제됨. 월령별 수면법은 `gemini_client.py`의 `_AGE_SLEEP_POLICY` 상수로 관리하고, 호출마다 해당 월령의 허용 수면법만 user prompt에 주입한다. 원더윅스 해당 여부도 `_WONDER_WEEKS` 상수로 Python 사전 계산 후 주입.

### 조립 순서

```python
_SYSTEM_PROMPT = (
    system.md + "---" + knowledge.md + "---" + reasoning.md
    + "---"
...
- "가능성"만 표현, 확정 금지
- 데이터와 연결되지 않은 해석 금지
- 허용 수면법은 user prompt에서 주입된 값만 사용
...
1차 호출 → JSON 파싱 실패 → "유효한 JSON만 출력" 지시 후 재호출
              └─ 재호출도 파싱 실패 → 예외 raise (호출자에 전파)
1차 호출 → 파싱 성공 → 필드 누락 발견 → 누락 필드 명시 후 재호출
              └─ 재호출 파싱 성공 → 여전히 누락 → 에러 로깅 후 현재 결과 반환
              └─ 재호출 파싱 실패 → 에러 로깅 후 이전 파싱 결과 반환 (서비스 중단 방지)
```
