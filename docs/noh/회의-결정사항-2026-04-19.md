...
| 의제 | 결정 내용 | 담당자 | 상태 |
|------|-----------|--------|------|
| #1 수면 점수 산식 | 미결 — 도메인 전문가 확정 대기 | 도메인 전문가 | ⏳ 미결 |
| #2 취침/기상 시각 | 미결 — 엠테이크 측 회신 대기. 불가 시 해당 UI 제거(C안) | 백엔드 + EMTAKE | ⏳ 엠테이크 확인 필요 |
| #3 AI 출력 형식 | **B안(구조화 JSON) 채택** — `ai_comment: [{type, icon, title, text}]`, `sleep_guide`, `age_kick` 필드로 구현 완료 | noh | ✅ 완료 (2026-04-26) |
| #4 발달 가이드 방식 | **B안(Gemini 생성) 채택** — `age_kick: {title, text, is_wonder_weeks}` 필드로 구현 완료 | noh | ✅ 완료 (2026-04-26) |
| #5 보존 기간 | **연장 완료** — Generated_Reports 5주 / Weekly_Data 12주 (AI 고도화 컨텍스트 용도) | noh | ✅ 완료 (2026-04-26) |
