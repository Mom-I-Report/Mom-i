...
| ✅ 완료 | `sleep_guide: null` 처리 버그 수정 | 04-26 수정 완료 |
| ✅ 완료 | `google-generativeai` → `google-genai` SDK 마이그레이션 | 04-27 수정 완료 |
| ✅ 완료 | CORS `allow_credentials=True` + `allow_origins=["*"]` 위반 | 05-12 수정 완료 |
| ✅ 완료 | 스케줄러 로그 메시지 오류 | 05-12 수정 완료 |
...
| `body_temp.*` | float | °C (델타값) | EMTAKE `Temp` — 절대 체온 아닌 기준치 대비 상승분 | 맘아이 서버 |
...
- `GEMINI_API_KEY`: Google AI Studio → API Keys에서 생성. 모델명 `gemini-3.1-flash-lite` (GA 모델, 별도 접근 권한 불필요).
...
```python
# 현재 (전체 허용 + credentials 버그 수정 완료 — 05-12)
allow_origins=["*"],
allow_credentials=False,   # Bearer JWT 사용, 쿠키 불필요
...
### 5. `on_event("startup")` → `lifespan` 방식 전환
...
| 항목 | 일자 | 내용 |
|------|------|------|
| sleep_guide null 처리 버그 수정 | 04-26 | 0~2개월 아기 시 `SleepGuide(**None)` → `TypeError` 수정 |
| 프롬프트 6파일 분리 | 04-26 | system / knowledge / age_policy / reasoning / output_format / input_template |
| kick_action 필드 추가 | 04-26 | SleepGuide에 추가, 프론트 렌더링 추가 |
| Gemini 모델 변경 | 04-26 | `gemini-2.5-flash` → `gemini-3-flash-preview` |
| AAP 기준 코드 반영 | 04-26 | 수면시간·호흡수·체온·뒤척임 판정 모두 코드에서 직접 계산 |
| 캐시 로직 | 04-26 | 동일 주차 재호출 시 Gemini 재호출 없이 DB 캐시 반환 |
| rolling 삭제 | 04-26 | 매주 월요일 10:00 KST 자동 삭제 (Generated_Reports 5주 / Weekly_Data 12주) |
| 인증 구조 | 04-26 | X-API-Key (서버→서버) + JWT Bearer (앱→서버) 완성 |
| google-genai SDK 마이그레이션 | 04-27 | `google-generativeai` → `google-genai`, FutureWarning 해소 |
| 관리자 기기 API 추가 | 04-27 | `GET /admin/devices`, `GET /admin/devices/{ser_no}/reports` |
| CORS credentials 버그 수정 | 05-12 | `allow_credentials=False` (Bearer JWT 사용, 쿠키 불필요) |
| 스케줄러 로그 메시지 수정 | 05-12 | "3주 초과" → "Generated_Reports 5주 / Weekly_Data 12주" |
| EMTAKE 전체 필드 통합 | 05-12 | Humidity·Bright·sessions·wakeup_count·device_status 반영 |
| 프론트엔드 React/Vite 전환 | 05-11~12 | Demo/Admin 페이지 TSX 전환, PDF 다운로드 개선 |
